var gadgetController = function() {
  return {
    init: function(gadget) {
      ...
      var _invalidUrl = { content: "Invalid SurveyMonkey URL" };
      var _invalidHeight = { content: "Invalid Height" };
      ...
      function setHeight(domObj, newHeight, heightField) {
        ...
        domObj.animate({ height: newHeight }, 1000, function() { gadgets.window.adjustHeight(); });
      }
      ...
      function invalidInput(domInput) {
        ...
        setTimeout( function() { domInput.parent().parent().removeClass("has-error"); domInput.parent().parent().find("label").popover("hide"); }, POPOVER_TIME_MS);
      }
      ...
      function render() {
        ...
        if(!_inited) {
          _settingsPane.find("#settings-form").find("label").each(function(idx) { $(this).popover(_popoverOptionsArray[idx]); });
          _settingsPane.on("hidden.bs.collapse", function() { $("#input-group-actions").show(); $("#settings-btn").show(); gadgets.window.adjustHeight(); });
          _settingsPane.on("shown.bs.collapse", function() { $("#input-group-actions").hide(); $("#settings-btn").hide(); setTimeout(function() { gadgets.window.adjustHeight(); }, 1000); storeSettings(); });
      ...
      typeof(gadgets) != "undefined" && gadgets && gadgets.util.registerOnLoadHandler(init);
	  }
  };
}();
