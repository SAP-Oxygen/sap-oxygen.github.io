        ...
        makeWidget : function(options) {
                ...
                var form = {
                    ...
                    onShow : function(dlg) {
                        ...
                        var _setOwnerData = function(owner) {
                            ...
                            var innerHtml = '<li class = "token-input-token-facebook"><p>'+name.escapeHTML()+
                            '</p><span class="token-input-delete-token-facebook">'+'×</span></li>';
            ...
            function _makeAgendaSetupForm() {
                ...
                function _getFormHtml() {
                    ...
                    sb.push('<div class="agenda_label">' + CSTAR.t("agenda.event_date") + ':</div>');
                    sb.push('<input id="event_date_input" type="text" size="32" name="event_date_input" class="blank" value="' + CSTAR.t("agenda.enter_date") + '"/>');
                    sb.push('<div id="calendar_icon_id" class="calendar_icon"></div>');
                    sb.push('</div>');
                    
                    sb.push('<div id="event_start_time">');
                    sb.push('<div class="agenda_label">' + CSTAR.t("agenda.event_start_time") + ':</div>');
                    if (military_time) {
            ...
            function _getValidItemHtml(i, item) {
                ...
                sb.push('</div><td class="cell col-2">');
                sb.push('<div class="agenda-trash">&times;</div>');
                sb.push('<div class="agenda-row-time">');
                if(item.time) {
                    for(j = 0; j < i; j++) {
                ...
                sb.push(" " + jamApp.t("agenda.x_minutes", {count: item.time}));
                sb.push('</div>');
                sb.push('<div class="topic-and-owner">');
                sb.push('<span class="topic">');
            ...
            function _getVoidItemHtml(i) {
                ...
                sb.push('"><td class="cell col-1">');
                sb.push(_getItemIndexInnerHtml(i));
                sb.push('</td><td class="cell agenda-clickadd col-2">' + CSTAR.t("agenda.click_add_new_agenda_topic") + '</td></tr>');
                return sb.join('');
            }
            ...
            function _getItemInfoHtml(startTime, endTime){
                ...
                sb.push('<span class="agenda-info-when"><span class="bold">' + CSTAR.t("agenda.event_date") + ':</span> '+ lipstick.formatDate(date_format_pattern, new Date(startTime), date_format_symbols) +'</span>');
            ...
            function _bindItem (pos, jI, jIM) {
                ...
                _bindItemTime (jI.find('.col-2'), pos);
...
*/
            ...
            function _doUpdate (data) {
                if (data) {
                    ...
                    var start = data.agenda.startTime;
                    if (data.agenda.timezoneOffset) {
                        start += data.agenda.timezoneCorrection * 1000;
                    }
        ...
        makePBEHandler : function(widget) {
            var _pbe = {
                ...
                publish : function(msg) {
                    ...
                    jamApp.currentAgenda.pbe_data.push(msg);
                },
